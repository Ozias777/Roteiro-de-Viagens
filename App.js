...
// Obtém a altura da barra de status para uso no estilo
const statusBarHeight = StatusBar.currentHeight;
// Colocar 'SUA_API_KEY' (Chave de api da opneAI)
const KEY_GPT = 'SUA API KEY';

export default function App() {
  // Estados para armazenar dados de entrada do usuário e resultados da API
  const [city, setCity] = useState("");
  const [days, setDays] = useState(1);
  const [loading, setLoading] = useState(false);
  const [travel, setTravel] = useState("");

  // Função para lidar com a geração do roteiro
  async function handleGenerate() {
    ...
    // Cria o prompt para enviar à API do OpenAI
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.heading}>Roteiro de Viagens</Text>
